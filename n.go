...
const (
	...
	compressDef   = Dm_build_364
	compressIDDef = Dm_build_365
...
func (c *DmConnector) setAttributes(props *Properties) error {
	...
	c.bufPrefetch = props.GetInt(BufPrefetchKey, c.bufPrefetch, int(Dm_build_349), int(Dm_build_350))
...
func (c *DmConnector) parseOsAuthType(props *Properties) error {
	...
	if c.user == "" && c.osAuthType == Dm_build_342 {
		c.user = "SYSDBA"
	} else if c.osAuthType != Dm_build_342 && c.user != "" {
		return ECGO_OSAUTH_ERROR.throw()
	} else if c.osAuthType != Dm_build_342 {
